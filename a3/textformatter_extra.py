...
#added word count. use it with ".WC" to get the number of words in the file. It will only count the formatted words (not the commands). Place .WC under a block
# of text to get the number of words above it. Word count will be printed out before the file
#added indenting. use it with ".IN indent" where indent is the number of spacing a line will be indented by. Once called, the first sentence of the next
#paragraph underneath it will be indented.
#added replace function. Use it with ".RP word_that_will_be_replaced word_to_replace_with"
...
class Formatter(object):
                        ...
                        for line in the_input:
                                ...
                                if re.match("^\.LW (\d+)", line):
                                        ...
                                        try:
                                                ...
                                                self.print_word_after_msg()
                                                return []
                                elif re.match("^\.WC$", line):
										print("The word count is", self.word_c)
                                elif re.match("^\.IN (\d+)", line):
                                        self.indent = self.get_value(array)
                                        self.first_word = 1
                                elif re.match("^\.RP (\w+) (\w+)", line):
                                        self.replaced = array[1]
                                        self.word_replace_with = array[2]
                                        del array[2], array[1], array[0]
                                #format if it's turned on
                                elif (self.formatting ==1):
                                        for i in range(len(array)):
                                                array[i] = re.sub(self.replaced, self.word_replace_with, array[i])
                                        for word in array:
        ...
        def put_word_in_front(self, word):
                self.word_c = self.word_c + 1
                if (self.first_word == 1 and self.new_line ==1):
                        self.first_word = 0
                        self.new_line = 0
                        [self.line_w.append("") for x in range(self.indent)]
                [self.line_w.append("") for x in range(self.margin)]
                self.line_w.append(word)
                space_left = self.width - len(word) - (self.margin) - self.indent
                return space_left
